	...
	// Axios: attach ID token if logged in
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					<div className="flex h-16 items-center justify-between">
						...
						<div className="flex items-center gap-3">
							<Link to="/appointments" className="hidden sm:inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-white hover:bg-blue-700 focus:ring-4 focus:ring-blue-300">Book Session</Link>
							{user ? (
								<div className="flex items-center gap-2">
									<span className="text-sm text-gray-600">{user.email}</span>
									<button onClick={() => signOut(auth)} className="text-sm rounded-lg border px-3 py-1 hover:bg-gray-50">Logout</button>
								</div>
							) : (
								<button onClick={() => navigate('/login')} className="text-sm rounded-lg border px-3 py-1 hover:bg-gray-50">Login</button>
							)}
						</div>
					</div>
				</div>
	...
	export default function App() {
		...
		// Example routing logic (replace with your router if using react-router)
